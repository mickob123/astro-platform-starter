...
export async function handleProcessInvoice(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (skip_vision && pdf_storage_path) {
      console.log("skip_vision=true: skipping PDF download, will classify from email text only");
    }

    if (!skip_vision && pdf_storage_path && typeof pdf_storage_path === "string" && pdf_storage_path.length > 0) {
      ...
      try {
        ...
        if (downloadError) {
          ...
          if (bytes.length > MAX_PDF_BYTES) {
            console.warn(`PDF too large (${bytes.length} bytes > ${MAX_PDF_BYTES}), skipping vision processing`);
          } else {
          ...
          // Clean up temp file regardless of size
    ...
    if (skip_vision && attachment_base64) {
      console.log("Vision processing skipped (skip_vision=true), using text-only classification");
    }
