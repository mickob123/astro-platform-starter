...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // --- Rate limiting: max 30 classify calls per customer per hour ---
    ...
    if (!rlError && recentCalls !== null && recentCalls >= 30) {
      return new Response(
        JSON.stringify({ error: "Rate limit exceeded", detail: "Maximum 30 classify calls per hour.", retry_after_seconds: 300 }),
