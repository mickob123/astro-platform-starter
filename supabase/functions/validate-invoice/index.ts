...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // --- Rate limiting: max 60 validate calls per customer per hour ---
    ...
    if (!rlError && recentCalls !== null && recentCalls >= 60) {
      return new Response(
        JSON.stringify({ error: "Rate limit exceeded", detail: "Maximum 60 validate calls per hour.", retry_after_seconds: 300 }),
